...
import 'package:pickaboo/presentation/ui/widgets/brand_product_page/inline_filter.dart';
import 'package:pickaboo/presentation/ui/widgets/common/app_error_view.dart';

class BrandProductResults extends StatefulWidget {
  final bool isGridView;

  final void Function(ProductEntity)? onProductSelected;

  const BrandProductResults({
    super.key,
  ...
  @override
  State<BrandProductResults> createState() => _BrandProductResultsState();
}

class _BrandProductResultsState extends State<BrandProductResults> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<BrandProductsBloc, BrandProductsState>(
  ...
  List<Widget> _buildRows(
    ...
    for (int start = 0; start < items.length; start += columns) {
      ...
      rows.add(
        Padding(
          key: ValueKey('brand_row_${rowItems.first.id}'),
      ...
      if (productsSoFar % _chunkSize == 0 && productsSoFar < items.length) {
        ...
        rows.add(
          Padding(
            key: ValueKey('brand_inline_filter_$filterIndex'),
            padding: EdgeInsets.only(bottom: 12.h),
            child: BrandInlineFilter(filterIndex: filterIndex),
          ),
        );
      }
    }
  ...
  void _maybeFetchNextPage(BuildContext context, BrandProductsState state) {
    final paging = state.pagingState;
    if (!paging.hasNextPage || paging.isLoading) return;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<BrandProductsBloc>().add(
        BrandProductsEvent.getProducts(brandKey: state.brandKey),
      );
    });
  }
            ...
            return RepaintBoundary(
              child: Column(
                children: [
                  ...
                  SizedBox(height: 8.h),
                  BrandInlineFilter(filterIndex: filterIndex),
                ],
              ),
            );
          }
  ...
  Widget _buildErrorState(BuildContext context, BrandProductsState state) {
    return AppErrorView(
      ...
      onRetry: () {
        context.read<BrandProductsBloc>().add(
          BrandProductsEvent.refresh(brandKey: state.brandKey),
        );
      },
    );
  }
