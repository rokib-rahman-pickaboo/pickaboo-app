...
List<CategoryBannerGroup> groupBannersForCategory(
  ...
  final matched = banners.where((banner) {
    ...
    if (banner.forHomepage != 0) return false;
    if (!banner.linksToCategory) return false;

    final bannerCategorySlug = banner.identifier;

    return bannerCategorySlug == categorySlug;
  }).toList();
...
class CategoryBannerSection extends StatelessWidget {
  ...
  const CategoryBannerSection({super.key, required this.categorySlug});

  @override
  Widget build(BuildContext context) {
    final banners = context.select<BannerBloc, List<BannerEntity>>(
      (bloc) => bloc.state.banners,
    );

    final groups = groupBannersForCategory(banners, categorySlug);
