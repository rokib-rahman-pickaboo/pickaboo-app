  ...
  Future<void> _onLoad(_Load event, Emitter<RecommendedProductsState> emit) async {
    ...
    await result.fold(
      ...
      (recommendedProducts) async {
        ...
        final initialList =
            recommendedProducts.sellerRecommendedProducts.map((p) {
        ...
        emit(RecommendedProductsState.loaded(
          recommendedProducts.copyWith(
            sellerRecommendedProducts: initialList,
          ),
        ));
        ...
        if (hasCacheEnrichment && !isClosed) {
          emit(RecommendedProductsState.loaded(
            recommendedProducts.copyWith(
              sellerRecommendedProducts: List<ProductEntity>.from(itemsToEnrich),
            ),
          ));
        }
